...
struct mesh_render_job : public core::Job_Loop {
    ...
    void Execute( uint64_t index ) {
        render_command_t*   render_command = &commands[index];
...
#if 1
     mesh_render_job job;
     job.position_vec = get_data_vec<Transform>();
     job.shader_vec = get_data_vec<Shader_Component>();
     job.mesh_vec = get_data_vec<Mesh_Component>();

     uint64_t oldsize = render_command_queue.size();
     render_command_queue.resize(oldsize + entity_count);
     job.commands = &render_command_queue[oldsize];
