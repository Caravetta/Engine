...
std::vector<Component_ID> get_components( Handle handle )
{
     ...
     std::vector<Component_ID> return_comps;
     return_comps.resize(data_node->n_components);
     memcpy(return_comps.data(), data_node->components, sizeof(Component_ID) * data_node->n_components);
     return return_comps;
}
...
void _add_handle_to_node( Handle handle, Comp_Data_Node& data_node )
{
     ...
     *p_handle = handle;
     ...
     for ( size_t ii = 0; ii < data_node.n_components; ii++ ) {
...
static Comp_Data_Node* _create_comp_data_node( Component_ID* ids, size_t n_ids, uint32_t id )
{
     ...
     new_data_node->components = (Component_ID*)malloc(sizeof(Component_ID) * n_ids);
     memcpy(new_data_node->components, ids, sizeof(Component_ID) * n_ids);
     new_data_node->n_components = n_ids;
     //new_data_node->components = ids; //JOSH NEEDS TO CHANGE
...
static void _add_comps_to_empty_ent( Handle handle, Component_ID* ids, size_t n_ids )
{
     uint32_t crc32_id = crc32(ids, sizeof(Component_ID) * n_ids);
     Comp_Data_Node* data_node = _comp_list_exists(crc32_id);
     if ( data_node != NULL ) {
          _add_handle_to_node(handle, *data_node);
     } else {
          data_node = _create_comp_data_node(ids, n_ids, crc32_id);
          _add_handle_to_node(handle, *data_node);
     }
}
...
void add_handle( Handle handle, Component_ID* ids, size_t n_ids )
{
     if ( n_ids == 0 ) {
          ...
          quick_sort_uint32(ids, 0, (n_ids - 1));
          _add_comps_to_empty_ent(handle, ids, n_ids);
     }
}
...
void remove_handle( Handle handle )
{
     ...
     if ( data_node.tail->n_ents == 0 ) {
          ...
          if ( data_node.tail == NULL ) {
               ...
               free(data_node.components);
               delete(&data_node);
          }
     }
}

Rc_t _add_comps_to_ent( Handle handle, Component_ID* ids, size_t n_ids )
{
     std::vector<Entity_Map>& entity_map = component_data_system->entity_map;
     uint32_t handle_idx = handle_index(handle);
     ...
     size_t n_tmp_ids = old_data_node.n_components + n_ids;
     Component_ID tmp_ids[n_tmp_ids];

     memcpy(tmp_ids, old_data_node.components, sizeof(Component_ID) * old_data_node.n_components);
     memcpy(&tmp_ids[old_data_node.n_components], ids, sizeof(Component_ID) * n_ids);

     quick_sort_uint32(tmp_ids, 0, (n_tmp_ids - 1));

     uint32_t crc32_id = crc32(tmp_ids, sizeof(Component_ID) * n_tmp_ids);
     p_data_node = _comp_list_exists(crc32_id);
     if ( p_data_node == NULL ) {
          // need to create a new comp data node
          p_data_node = _create_comp_data_node(tmp_ids, n_tmp_ids, crc32_id);
     }
     ...
     *p_handle = handle;
     ...
     // loop through all comps and init their mem
     for ( size_t ii = 0; ii < new_data_node.n_components; ii++ ) {
     ...
     for ( size_t ii = 0; ii < new_data_node.n_components; ii++ ) {
          if ( new_data_node.components[ii] == old_data_node.components[old_comp_idx] ) {
               ...
               old_comp_idx += 1;
               if ( old_comp_idx >= old_data_node.n_components ) {
...
Rc_t handle_add_components( Handle handle, Component_ID* ids, size_t n_ids )
{
     ...
     if ( n_ids > 0 ) {
          ...
          if ( entity_map[handle_idx].node_idx != 0 ) {
               std::vector<Component_ID> ids_vec; //JOSH need to fix
               ids_vec.resize(n_ids);
               memcpy(ids_vec.data(), ids, sizeof(Component_ID) * n_ids);

               rc = _add_comps_to_ent(handle, ids, n_ids);
          } else {
               _add_comps_to_empty_ent(handle, ids, n_ids);
          }
     }
...
Rc_t _remove_comps_from_ent( Handle handle, Component_ID* ids, size_t n_ids )
{
     std::vector<Entity_Map>& entity_map = component_data_system->entity_map;
     uint32_t handle_idx = handle_index(handle);
     ...
     std::vector<Component_ID> new_ids;
     new_ids.resize(old_data_node.n_components);
     memcpy(new_ids.data(), old_data_node.components, sizeof(Component_ID) * old_data_node.n_components);
     ...
     if ( p_data_node == NULL ) {
          // need to create a new comp data node
          p_data_node = _create_comp_data_node(new_ids.data(), new_ids.size(), crc32_id);
     }
...
{
     return _remove_comps_from_ent(handle, ids, n_ids);
}
...
size_t get_data_chunks( std::vector<Data_Chunk>& data_chunks, std::vector<Component_ID>& components )
{
     ...
     for ( const auto &entry : component_data_system->data_node_map ) {
          ...
          std::vector<Component_ID> comps;
          comps.resize(data_node.n_components);
          memcpy(comps.data(), data_node.components, sizeof(Component_ID) * data_node.n_components);

          if ( std::includes(comps.begin(), comps.end(), //TODO(JOSH): need to replace
...
void comp_data_system_debug_print( void )
{
     for ( const auto &entry : component_data_system->data_node_map ) {
          ...
          LOG("Components [ %zd ]", data_node.n_components);
          for ( size_t jj = 0; jj < data_node.n_components; jj++ ) {
