...
void Camera::set_transformation( Vector3f* translation, float rx, float ry, float rz, float scale )
{
    ...
    transformation_matrix.rotate(to_radians(rx), &vec_x);
    transformation_matrix.rotate(to_radians(ry), &vec_y);
    transformation_matrix.rotate(to_radians(rz), &vec_z);
...
void Camera::set_projection_matrix( float width, float height )
{
    ...
    float y_scale = (float)((1.0f / tan(to_radians(fov / 2.0f))) * aspectRatio);
