...
Rc_t register_archetype( Archetype archetype, std::string archetype_name )
{
    ...
    std::unordered_map<std::string, uint64_t>* archetype_map = Archetype_Manager::archetype_manager->archetype_map;
    ...
    if ( arche_idx == archetype_map->end() ) {
        ...
        for (uint64_t i = 0; i < archetype.used_component_ids.size(); i++) {
            ...
            comp_size = get_component_size((uint32_t)archetype.used_component_ids[i]);
